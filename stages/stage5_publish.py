...
def run(stage2_output: dict, stage3_output: dict, stage4_output: dict,
    ...
    # Post pinned comment to drive early engagement
    pinned_comment = stage3_output.get("pinned_comment")
    if pinned_comment:
        _post_pinned_comment(youtube, result["video_id"], pinned_comment)
...
def _post_pinned_comment(youtube, video_id: str, comment_text: str) -> None:
    try:
        ...
        logger.info("Stage 5 | Pinned comment posted: %s", comment_text)
    except Exception as e:
        logger.warning("Stage 5 | Could not post pinned comment: %s", e)
